...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-bold text-gray-800">New Package</h2>
            <p class="text-sm text-gray-500">Create a pre-configured solar system package.</p>
        </div>
    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required
                                placeholder="e.g. 3 kW Residential Starter Pack"
                        ...
                        <div>
                            ...
                            <input type="number" name="system_size_kw" value="{{ old('system_size_kw') }}" required
                        ...
                        <div>
                            ...
                            <select name="suitable_for" required
                                ...
                                <option value="{{ $sf }}" {{ old('suitable_for') === $sf ? 'selected' : '' }}>{{ ucfirst($sf) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1.5">Warranty (Years)</label>
                            <input type="number" name="warranty_years" value="{{ old('warranty_years') }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" rows="3"
                                ...
                                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300">{{ old('description') }}</textarea>
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <textarea name="includes" rows="4"
                                placeholder="400W Mono Perc Solar Panels&#10;3 kW On-grid Inverter&#10;Mounting Structure&#10;DC/AC Cables & Accessories&#10;Installation & Commissioning"
                                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300">{{ old('includes') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm" id="pkgItemsTable">
                            ...
                            <tbody id="pkgItemsBody" class="divide-y divide-gray-50">
                                {{-- rows injected by JS --}}
                            </tbody>
...
function addPkgRow(name = '', qty = 1, price = 0) {
    const idx = pkgRowIdx++;
        ...
        <td class="px-3 py-2">
            <div class="flex items-center gap-2">
                <select class="pkg-product-select border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300 w-36">
                    ...
                    ${pkgProducts.map(p => `<option value="${p.id}" data-price="${p.price}" data-name="${p.name}">${p.name}</option>`).join('')}
                </select>
                ...
                <input type="hidden" name="items[${idx}][product_id]" class="pkg-product-id">
            </div>
        </td>
